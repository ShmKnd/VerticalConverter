...
- **macOS 13 support** — Lowered deployment target from macOS 14.6 to macOS 13.0. SDR video conversion works fully on macOS 13
- **macOS 13: HDR graceful degradation** — HDR panel is grayed out with "macOS 14+" label on macOS 13. HDR video input is rejected with a clear error message (`hdrNotSupported`)
- **macOS 13: `.symbolEffect` compatibility** — Wrapped `.symbolEffect(.pulse)` in `PulseEffectModifier` with `if #available(macOS 14.0, *)` guard. Falls back to static icon on macOS 13
- **Demo edition: trial system** — 5 free full-quality encodes per 24-hour rolling window. Watermark applied after limit. Resets automatically every 24 hours. UI shows remaining free encodes or "Trial expired" status
- **Demo edition: `DemoUsageTracker`** — New `UserDefaults`-based tracker (`DemoWindowStartDate` / `DemoEncodeCount`) with auto-reset on 24h window expiry
- **Demo watermark: per-export capture** — Watermark decision captured in `VerticalVideoCompositor.staticShowsWatermark` at export start, ensuring consistent behavior throughout the export
- **About window redesign** — Replaced `orderFrontStandardAboutPanel` with custom borderless `NSWindow` (`titlebarAppearsTransparent`, `fullSizeContentView`). Shows app icon, tagline ("The smart landscape-to-portrait video converter."), version, GitHub repo link, X link, and scrollable full LICENSE text
- **GitHub link fix** — About window GitHub link now points to the repository (`https://github.com/ShmKnd/VerticalConverter`) instead of the developer profile
...
- **出力フォルダ選択** — 全エディションで出力先を明示指定可能（リセットボタンで「入力と同じ」に戻す）
- **設定ツールチップ** — 各設定行にマウスホバーで英語の説明を表示
- **エンコード中 GUI ロック** — 変換中はドロップゾーン・設定パネルを無効化（Cancel は有効）
- **完了サウンド** — エンコード完了時にシステムサウンド (Glass) を再生
- **About ウィンドウ** — カスタムウィンドウバーなしの About 画面。アプリアイコン・紹介文・GitHub/X リンク・ライセンス全文を表示
- **バージョン表示** — ヘッダーにアプリバージョンを表示
...
- **Privacy Manifest 追加** — `PrivacyInfo.xcprivacy` を作成・ビルドに追加（2024年5月以降必須）。トラッキングなし、収集データなし、Required Reason API の申告なし（UserDefaults 等の Required Reason API を使用していないため）
- **App Store 審査対応確認** — 以下の項目が準拠済みであることを検証：
  - AppStore entitlements の App Sandbox（`com.apple.security.app-sandbox` + `files.user-selected.read-write`）
...
- **macOS 13 対応** — デプロイメントターゲットを macOS 14.6 → macOS 13.0 に引き下げ。SDR 動画変換は macOS 13 で完全動作
- **macOS 13: HDR グレースフルデグラデーション** — macOS 13 では HDR パネルを「macOS 14+」ラベル付きでグレーアウト表示。HDR 動画入力時は明確なエラーメッセージ (`hdrNotSupported`) で拒否
- **macOS 13: `.symbolEffect` 互換対応** — `.symbolEffect(.pulse)` を `PulseEffectModifier` でラップし `if #available(macOS 14.0, *)` でガード。macOS 13 では静的アイコン表示にフォールバック
- **Demo 版: トライアルシステム** — 24 時間ローリングウィンドウごとに 5 回のフリーエンコード（フル品質）。制限超過後はウォーターマーク付き。24 時間経過で自動リセット。UI に残りフリーエンコード回数または「Trial expired」ステータスを表示
- **Demo 版: `DemoUsageTracker`** — `UserDefaults` ベースのトラッカー（`DemoWindowStartDate` / `DemoEncodeCount`）を新設。24 時間ウィンドウ満了時に自動リセット
- **Demo ウォーターマーク: エクスポート時キャプチャ** — ウォーターマーク表示判定をエクスポート開始時に `VerticalVideoCompositor.staticShowsWatermark` にキャプチャし、エクスポート中の一貫性を保証
- **About ウィンドウ刷新** — `orderFrontStandardAboutPanel` をカスタムボーダーレス `NSWindow`（`titlebarAppearsTransparent`, `fullSizeContentView`）に置換。アプリアイコン・紹介文（"The smart landscape-to-portrait video converter."）・バージョン・GitHub リポジトリリンク・X リンク・スクロール可能な LICENSE 全文を表示
- **GitHub リンク修正** — About ウィンドウの GitHub リンクを開発者プロフィール → リポジトリ (`https://github.com/ShmKnd/VerticalConverter`) に変更
...
<p align="center"><a href="#日本語">⬆ トップへ戻る</a></p>
