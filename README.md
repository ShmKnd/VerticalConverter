...
- **PAR (Pixel Aspect Ratio) Correction** — AVCHD 1440×1080 (PAR 4:3) is correctly treated as 1920×1080 display size
- **Dock Progress Bar** — Shows conversion progress in the Dock

> **Supported containers**: MP4 and MOV are fully supported. MPEG-TS (`.mts`, `.m2ts`, `.ts`) inputs are rejected with an explicit error message due to AVFoundation instability with interlaced MPEG-TS sample delivery.
...
| Supported Input | MP4 / MOV (H.264, H.265, hev1/hvc1, AVCHD 1440×1080). MPEG-TS (.mts/.m2ts/.ts) is **not supported** |
| Smart Framing Analysis | Vision requests are downscaled to ≤960px long side for performance |
...
```
[Input Validation]
  MPEG-TS (.mts/.m2ts/.ts) → unsupportedFormat error (early reject)
  DNxHD/DNxHR              → unsupportedFormat error (early reject)

[Pre-processing]
  hev1 → hvc1 remux ─────────────────────┐
  PAR-aware display size (e.g. 1440×1080   │
    pixel → 1920×1080 display via PAR 4:3) │
  HDR metadata detection ─────────────────┘
                            ▼
[Pass 1] SmartFramingAnalyzer (when enabled)
  Vision input downscaled to ≤960px (long side) for performance/stability
  Person detection → IOU tracking → main subject estimation → interpolation → EMA → hold & follow
                            ▼
[Pass 2] VerticalVideoCompositor
  PAR correction transform applied to source CIImage
...
- **Crop Position Adjustment** — Added horizontal crop position slider in both the main settings panel and Preview window. In the main panel the slider is always accessible (including batch mode) and is grayed out when no video is loaded, when Fit W is selected, or when Smart Framing is enabled. Allows shifting the crop center left/right for Fit H, Square, 4:3, and 3:4 modes
- **Crop Position UI Polish** — Reset button replaced with a ↺ icon placed to the right of the slider (consistent with the Output row clear button). Main panel label shortened to "Center Pos". Preview panel label unified to "Center Position" with a matching icon and ↺ reset button. All setting rows (Resolution→Output) now share the same `settingRow` helper and a common minimum row height for visual consistency
- **Version text removed from header** — The `v1.x.x` version string below the subtitle has been removed to reduce visual clutter
- **Settings Persistence** — All conversion settings (resolution, frame rate, codec, container, bitrate, bitrate mode, crop mode, crop position, smart framing, HDR settings) are now saved via UserDefaults and automatically restored on next launch
- **Output Directory Persistence** — Selected output folder is saved as a Security-Scoped Bookmark and restored across app launches (AppStore edition)
- **Privacy Manifest Updated** — Added `NSPrivacyAccessedAPICategoryUserDefaults` (reason: CA92.1) to `PrivacyInfo.xcprivacy` since the app now uses UserDefaults for settings persistence
- **AppStore Entitlement** — Added `com.apple.security.files.bookmarks.app-scope` for output directory bookmark persistence
- **Smart Framing: Vision Analysis Downscale** — Vision person detection requests are now downscaled to ≤960px (long side) before analysis. Improves stability and performance on 4K and AVCHD inputs without affecting detection accuracy
- **PAR (Pixel Aspect Ratio) Correction** — AVCHD 1440×1080 (PAR 4:3, DAR 16:9) is now correctly treated as 1920×1080 display size throughout the pipeline. Fixes squashed output and incorrect Smart Framing on non-square-pixel sources
- **MPEG-TS Input Rejected** — `.mts`, `.m2ts`, `.ts` inputs now return an explicit `unsupportedFormat` error at validation. These containers caused unstable `AVAssetReader` sample delivery with interlaced AVCHD content. Convert to MP4 first using ffmpeg or Handbrake
- **Version updated to 1.0.2** — MARKETING_VERSION / CURRENT_PROJECT_VERSION updated across all 6 configurations
...
- **クロップ位置調整** — クロップ位置（横方向）スライダーをメイン設定パネルと Preview ウィンドウの両方に追加。メインパネル側はバッチモード時も常にアクセス可能で、動画未読み込み時・Fit W 選択時・Smart Framing 有効時はグレーアウト。Fit H / Square / 4:3 / 3:4 モードでクロップ中心を左右にシフト可能
- **クロップ位置 UI 改善** — リセットボタンを「Reset」テキストから ↺ アイコンに変更しスライダーの右端へ移動（Output 行の ✕ ボタンと位置を統一）。メインパネルのラベルを「Center Pos」に短縮、Preview パネルのラベルを「Center Position」に統一しアイコンも揃えた。Resolution〜Output の全設定行が同一の `settingRow` ヘルパー経由になり最小行高さも統一
- **ヘッダーのバージョン表記を削除** — サブタイトル下の `v1.x.x` 文字列を削除してヘッダーを簡潔に
- **設定の永続化** — 全変換設定（解像度、フレームレート、コーデック、コンテナ、ビットレート、ビットレートモード、クロップモード、クロップ位置、スマートフレーミング、HDR 設定）を UserDefaults で保存し、次回起動時に自動復元
- **出力フォルダの永続化** — 選択した出力フォルダを Security-Scoped Bookmark として保存し、アプリ再起動後も復元（AppStore 版）
- **Privacy Manifest 更新** — UserDefaults を使用するため `NSPrivacyAccessedAPICategoryUserDefaults`（理由: CA92.1）を `PrivacyInfo.xcprivacy` に追加
- **AppStore Entitlement 追加** — 出力ディレクトリのブックマーク永続化のため `com.apple.security.files.bookmarks.app-scope` を追加
- **スマートフレーミング Vision 解析縮小** — Vision の人物検出リクエストを長辺 960px 以下に縮小してから解析するようになった。4K・AVCHD 入力での安定性とパフォーマンスを向上（検出精度への影響なし）
- **PAR（非正方形ピクセル）補正** — AVCHD 1440×1080（PAR 4:3・DAR 16:9）を表示サイズ 1920×1080 として正しく扱うようになった。非正方形ピクセル素材での縦長変換の歪みとスマートフレーミングの誤動作を修正
- **MPEG-TS 入力を拒否** — `.mts` / `.m2ts` / `.ts` 入力はバリデーション時点で `unsupportedFormat` エラーを返すようになった。これらのコンテナはインターレース AVCHD コンテンツで `AVAssetReader` のサンプル供給が不安定なため非対応とした。事前に ffmpeg や Handbrake で MP4 に変換してから使用すること
- **バージョンを 1.0.2 に更新** — MARKETING_VERSION / CURRENT_PROJECT_VERSION を全 6 構成で更新
