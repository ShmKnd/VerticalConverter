...
- **Crop Position Adjustment** — Added horizontal crop position slider in Preview window. Allows shifting the crop center left/right for Fit H, Square, 4:3, and 3:4 modes (grayed out for Fit W where horizontal shift is not applicable)
...
- **クロップ位置調整** — Preview ウィンドウにクロップ位置（横方向）スライダーを追加。Fit H / Square / 4:3 / 3:4 モードでクロップ中心を左右にシフト可能（Fit W では横シフト不要のためグレーアウト）
