...
- **About Window** — View version, license, and links from "About Vertical Converter" in the menu bar
...
| Item | Details |
|:--|:--|
| Supported OS | macOS 14.0+ (Apple Silicon only) |
...
| **VerticalConverter (Demo)** | Debug Demo / Release Demo | `EDITION_DEMO` | Demo version. Watermark on output. DEMO badge in UI |
...
- Demo edition outputs include a "DEMO" watermark
...
- **Smart Framing: Vision request caching** — `VNDetectHumanBodyPoseRequest` / `VNDetectFaceRectanglesRequest` instances are now created once and reused across frames instead of being allocated per-frame. Reduces memory churn and CPU overhead on long videos
  - `SmartFramingAnalyzer.detectAllPositions()`: requests created as local variables and passed to `detectRawPersons()`
  - `VerticalVideoCompositor`: requests cached as instance properties (`cachedBodyRequest` / `cachedFaceRequest`)
...
- **About ウィンドウ** — メニューバーから「About Vertical Converter」でバージョン・ライセンス・リンクを表示
...
| **VerticalConverter (Demo)** | Debug Demo / Release Demo | `EDITION_DEMO` | デモ版。出力にウォーターマーク合成。UIに DEMO バッジ |
...
- Demo 版は出力動画に「DEMO」ウォーターマークが入ります
...
- **スマートフレーミング: Vision リクエストキャッシュ最適化** — `VNDetectHumanBodyPoseRequest` / `VNDetectFaceRectanglesRequest` をフレームごとに生成する代わりに、一度生成して再利用するよう変更。長時間動画でのメモリ消費・CPU 負荷を軽減
  - `SmartFramingAnalyzer.detectAllPositions()`: リクエストをローカル変数で生成し `detectRawPersons()` に渡す形に変更
  - `VerticalVideoCompositor`: リクエストをインスタンスプロパティ（`cachedBodyRequest` / `cachedFaceRequest`）としてキャッシュ
